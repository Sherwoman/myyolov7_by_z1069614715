        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if type(iou) is tuple:
                    lbox += (iou[1].detach() * (1 - iou[0].squeeze())).mean()
                    iou = iou[0]
                else:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                ### iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, CIoU=True) 0221_0030_7tiny_8bs250ep_9530_6670
                iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, EIoU=True, Focal=True, gamma=0.5)  # FocalEIoU
                # lbox += (1.0 - iou).mean()  # iou loss
                if type(iou) is tuple:
                    lbox += (iou[1].detach() * (1 - iou[0].squeeze())).mean()
                    iou = iou[0]
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                ######241102####from_which_layer.append(torch.ones(size=(len(b),)) * i)
                from_which_layer.append((torch.ones(size=(len(b),)) * i).to(torch.device(device)))  ######241102####
            ...
            fg_mask_inboxes = fg_mask_inboxes.to(torch.device(device))  ######241102#####
